...
def test(session: nox.Session) -> None:
    ...
    session.run("pytest", "--junit-xml", str(output_file), "--cov=src.eventoolkit", "--cov-report=term-missing")
...
def lint(session: nox.Session) -> None:
    ...
    session.run("black", "--check", "tests", "src/eventoolkit")
